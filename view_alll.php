...
$(document).ready(function() {
    updateWishlistCount();
        ...
        $("#signupForm").submit(function(e) {
            ...
            // First check pincode availability
            ...
            if (pincode.length !== 6) {
                ...
                showToast('error', 'Please enter a valid pincode and check availability');
                return false;
            }
            
            // Check if pincode is available (from the message shown)
            if (pincodeMessage.includes("unavailable") || pincodeMessage.includes("do not deliver") || pincodeMessage === "") {
                // Make the pincode field and message red
                $("#pincode").addClass('is-invalid');
                $("#pincodeResult").html('<span class="text-danger"><i class="fas fa-times-circle"></i> Delivery not available in your area. Registration not possible.</span>');
                
                // Show alert
                alert("Sorry, we don't deliver to your area yet. Registration is not possible for this pincode.");
                
                // Show toast message
                showToast('error', 'Sorry, we currently don\'t deliver to your pincode. Registration not possible.');
                return false;
            }
                ...
                success: function(response) {
                    if (response.trim() === "success") {
                        ...
                        if (response.includes("Email already exists")) {
                            showError("#signupEmail", response);
                        } else if (response.includes("Phone number already exists")) {
                            showError("#phone", response);
                        } else {
                            showError("#signupPassword", response || "Invalid signup data");
                        }
